...
class Propiedad(models.Model):
    TIPO_PROPIEDAD_CHOICES = [
        ...
        ('local', 'Local'), ('terreno', 'Terreno'), ('otro', 'Otro'),
    ]
    TIPO_OPERACION_CHOICES = [('venta','Venta'),('alquiler','Alquiler'),('temporario','Temporario')]
    ESTADO_PUBLICACION = [('activa','Activa'),('pausada','Pausada'),('finalizada','Finalizada')]
    ...
    def save(self, *args, **kwargs):
        ...
        # Presentación bonita + normalizados
    ...
    def precio_display(self):
        if self.precio_usd is not None:   return f"US$ {self.precio_usd:,.0f}"
        if self.precio_pesos is not None: return f"$ {self.precio_pesos:,.0f}"
        return "Consultar"
